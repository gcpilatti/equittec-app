...
function BadgeDias({ dias }) {
  ...
  if (dias === 0) return <span className="text-[10px] font-bold text-orange-500 bg-orange-50 px-2 py-0.5 rounded-full">Hoje</span>;
  if (dias <= 7)  return <span className="text-[10px] font-bold text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded-full">Em {dias}d</span>;
  return <span className="text-[10px] font-bold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Em {dias}d</span>;
}
          ...
          return (
            <div key={v.id} className="px-4 py-3">
              <div className="flex items-start justify-between gap-2">
                <p className="font-semibold text-gray-900 text-sm">{v.clienteNome || '—'}</p>
                <p className="text-gray-600 text-sm shrink-0">{formatarData(v.proximoContato)}</p>
              </div>
              <div className="flex items-center gap-2 mt-1">
                <BadgeDias dias={dias} />
                {v.vendedorNome && (
                  <span className="text-[11px] text-gray-400">{v.vendedorNome}</span>
                )}
              </div>
              {v.obsProximoContato && (
                <p className="text-orange-500 text-sm mt-1 line-clamp-2">{v.obsProximoContato}</p>
              )}
            </div>
          );
        })}
